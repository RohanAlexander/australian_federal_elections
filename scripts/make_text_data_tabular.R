...
clean_adams_voting_data <-
	function(name_of_input_text_file) {
		...
		votingData <-
			read_lines(name_of_input_text_file) # Used to read txt files
		votingData <-
			tibble(raw_data = votingData) # Convert to rectangular
		...
		if (file_txt_name == "1996repsvic") {
			votingData <-
				rbind(votingData[1:1172,],
							c("George Mitsou                            1,683    2.5", "1996repsvic"),
							votingData[1173:nrow(votingData),])
		}
		...
		votingData <- votingData %>%
			mutate(
				...
				# Find where there are at least three "=" in a row
		...
		if (file_txt_name == "1998repsnsw") {
			votingData$election_date[votingData$division == "NEWCASTLE, NSW"] <-
				ymd("1998-11-21")
		}
		
		if (file_txt_name == "1993repsqld") {
			votingData$election_date[votingData$division == "DICKSON, Qld"] <-
				ymd("1993-04-17")
		}
		...
		# Fix typos in the original data:
		votingData$raw_data[votingData$raw_data == "4th count: Hutchinson's 15,777 votes distributed"] <-
			"5th count: Hutchinson's 15,777 votes distributed"
		votingData$raw_data[votingData$raw_data == "8th count: Ellsmore's 19,593 votes distributed"] <-
		...
		if (are_there_preferences > 1) {
			votingData <- votingData %>%
				...
				ungroup() %>%
				filter((
					# First grab the first-preferences - these start at different points so need to use a point that is specific to the file
			...
			# This is just for the elections (mostly back in the day) when there weren't preferences.
			votingData <- votingData %>%
				group_by(division) %>%
				mutate(start = if_else(section == "TRUE", 1, 0),
							 counter = cumsum(start)) %>%
		...
		votingData_first_prefs <- votingData_first_prefs %>%
			mutate(
				...
				votes_share = str_remove(votes_share, "\\(?\\+[[:digit:]]?$"),
				# Looks for something like: "52.0 (-" or "35.3 (+0"
				votes_share = str_remove(votes_share, "\\(?\\-[[:digit:]]?$"),
				# Looks for something like: "52.0 (-" or "35.3 (-0"
				votes_share = str_remove(votes_share, "\\([[:space:]]?$"),
				# Looks for trailing "(" or "( )
				votes_share = str_remove(votes_share, "[[:space:]]{2,}0$"),
				# Looks for trailing "  0"
				swing = str_remove(swing, "^[[:digit:]][[:space:]]{2,}"),
				# Looks for leading numbers followed by two spaces "9  22.1"
				swing = str_remove(swing, "^[[:digit:]][[:space:]]?$"),
				# Looks for leading numbers followed by a space and nothing else "9 " or "9"
			)
		...
		votingData$raw_data <- str_replace_all(
			votingData$raw_data,
			c(
				"1,921 23.8" = "1,921  23.8",
				# WILMOT in 1922repstas
				"1,511 18.8" = "1,511  18.8",
				# WILMOT in 1922repstas
				"2,629 32.6" = "2,629  32.6",
				# WILMOT in 1922repstas
				"1,996 24.8" = "1,996  24.8",
				# WILMOT in 1922repstas
				"2,950 36.6" = "2,950  36.6",
				# WILMOT in 1922repstas
				"2,799 34.7" = "2,799  34.7",
				# WILMOT in 1922repstas
				"2,308 28.6" = "2,308  28.6",
				# WILMOT in 1922repstas
				"412 34.1" = "412  34.1",
				# NORTHERN TERRITORY in 1922repsnt
				"492 40.8" = "492  40.8",
				# NORTHERN TERRITORY in 1922repsnt
				"303 25.1" = "303  25.1",
				# NORTHERN TERRITORY in 1922repsnt
				"19,730 59.5" = "19,730  59.5",
				# MORETON in 1922repsqld
				"13,422 40.5" = "13,422  40.5",
				# MORETON in 1922repsqld
				"8,850 13.8" = "8,850  13.8",
				# LA TROBE in 1961repsvic
				"28,057 43.8" = "28,057  43.8",
				# LA TROBE in 1961repsvic
				"27,125 42.4" = "27,125  42.4",
				# LA TROBE in 1961repsvic
				"14,497 \\(26.8\\)" = "14,497  26.8",
				# SCULLIN in 1987repsvic
				"39,539 \\(73.2\\)" = "39,539  73.2",
				# SCULLIN in 1987repsvic
				"CLEELAND  \\*" = "CLEELAND *",
				# McEWEN in 1987repsvic
				"FISCHER  \\*" = "FISCHER *",
				# FARRER in 1987repsnsw
				"Chesterfield-Evans 2,336" = "Chesterfield-Evans  2,336",
				# NORTH SYDNEY in 2013repsby
				"Cambel            l2,041" = "Cambell            2,041",
				# PERTH in 1987repswa
				"Varide               l71" = "Varidel               71",
				# BEROWRA in 1990repsnsw
		...
		votingData$redistributed_vote_share <-
			str_remove(votingData$redistributed_vote_share, "\\)")
		...
		votingData <- votingData %>%
			mutate(
				...
				division_percent_voted = if_else(
					division_percent_voted == "unknown",
					"NA",
					division_percent_voted
				),
...
all_voting$state[all_voting$division == "NORTHERN TERRITORY"] <-
	"NT"
all_voting$state[all_voting$division == "AUSTRALIAN CAPITAL TERRITORY"] <-
	"ACT"
...
all_voting$division[all_voting$division == "KINGSFORD-SMITH"] <-
	"KINGSFORD SMITH"
...
all_voting <- all_voting %>%
	mutate(name = str_remove_all(
		name,
		"^Rt Hon Sir |^Rt Hon Dr |^Hon Sir |^Rt Hon |^Hon Dr |^Hon |^Sir |^Dr "
	))
# Remove stars
all_voting <- all_voting %>%
...
# Get rid of the exhausted votes. This is only an issue for:
# "1984repsact", "1984repsnsw", "1984repsnt", "1984repsqld", "1984repssa", "1984repstas", "1984repsvic", "1984repswa", "1984repsby", "1987repsact", "1987repsnsw", "1987repsnt", "1987repsqld", "1987repssa", "1987repstas", "1987repsvic", "1987repswa", "1987repsby", "1990repsact", "1990repsnsw", "1990repsnt", "1990repsqld", "1990repssa", "1990repstas", "1990repsvic", "1990repswa", "1990repsby", "1993repsact", "1993repsnsw", "1993repsqld", "1993repssa", "1993repstas", "1993repsvic", "1993repswa", "1993repsby", "1996repsact", "1996repsnsw", "1996repsnt", "1996repsqld", "1996repssa", "1996repstas", "1996repsvic", "1996repswa", "1996repsby"
# The others have a different formatting
all_voting <-
	all_voting %>%
	filter(!name %in% "exhausted")
...
all_voting$vote_redistribution_round[all_voting$vote_redistribution_round == "98"] <-
	"9"
rm(voting_rounds)

# Fix a date
# There's a typo in the NT file where it has the wrong date
all_voting$election_date[all_voting$txt_file == "1972repsnt" &
												 	all_voting$division == "NORTHERN TERRITORY"] <- ymd("1972-12-02")
...
# Fix a few
all_voting$votes_share[all_voting$votes_share == "71.4" &
											 	all_voting$original_name == "Wilson"] <- "64.48"
all_voting$votes_share[all_voting$votes_share == "28.6" &
											 	all_voting$original_name == "Payne"] <- "35.52"
...
all_voting$name[all_voting$name == "Birchall" &
									all_voting$division == "SWAN"] <- "Burchall"
all_voting$name[all_voting$name == "Bludworth" &
									all_voting$division == "NEWCASTLE"] <- "Budworth"
all_voting$name[all_voting$name == "Brotherton" &
									all_voting$division == "LALOR"] <- "Sheffield-Brotherton"
all_voting$name[all_voting$name == "Georgi" &
									all_voting$division == "FORREST"] <- "Giorgi"
all_voting$name[all_voting$name == "Jayawardene" &
									all_voting$division == "WENTWORTH"] <- "Jayawardena"
all_voting$name[all_voting$name == "Salter" &
									all_voting$division == "LEICHHARDT"] <- "Salier"
all_voting$name[all_voting$name == "Weare" &
									all_voting$division == "NEW ENGLAND"] <- "Wearne"
all_voting$name[all_voting$name == "Federicks" &
									all_voting$division == "RICHMOND"] <- "Fredericks"
all_voting$name[all_voting$name == "Westo" &
									all_voting$division == "WILMOT"] <- "Weston"
all_voting$name[all_voting$name == "Robin" &
									all_voting$division == "PATERSON"] <- "Robinson"
all_voting$name[all_voting$name == "Sainbury" &
									all_voting$division == "EDEN-MONARO"] <- "Sainsbury"
all_voting$name[all_voting$name == "Connley" &
									all_voting$division == "GIPPSLAND"] <- "Conley"
all_voting$name[all_voting$name == "Cambel" &
									all_voting$division == "PERTH"] <- "Campbell"
all_voting$name[all_voting$name == "Varide" &
									all_voting$division == "BEROWRA"] <- "Varidel"
all_voting$name[all_voting$name == "Carey" &
									all_voting$division == "FREMANTLE"] <- "Cary"
all_voting$name[all_voting$name == "Hetherich" &
									all_voting$division == "CASEY"] <- "Heatherich"
all_voting$name[all_voting$name == "Bekairis" &
									all_voting$division == "SCULLIN"] <- "Bekiaris"
all_voting$name[all_voting$name == "Petersen" &
									all_voting$division == "WATSON"] <- "Peterson"
all_voting$name[all_voting$name == "Heelam" &
									all_voting$division == "BRADFIELD"] <- "Neelam"
all_voting$name[all_voting$name == "Musolini" &
									all_voting$division == "HINDMARSH"] <- "Musolino"
all_voting$name[all_voting$name == "LAMP" &
									all_voting$division == "LONGMAN"] <- "LAMB"

all_voting$name[all_voting$name == "Owen Andrew" &
									all_voting$division == "CHIFLEY"] <- "Andrew Owen"
all_voting$name[all_voting$name == "Martin Hetherich" &
									all_voting$division == "CASEY"] <- "Martin Heatherich"
all_voting$name[all_voting$name == "Anthony Peterson" &
									all_voting$division == "GIPPSLAND"] <- "Anthony Petersen"
all_voting$name[all_voting$name == "Peter Procter" &
									all_voting$division == "FORREST"] <- "Peter Proctor"
all_voting$name[all_voting$name == "Joanne Clark" &
									all_voting$division == "FRASER"] <- "Joanne Clarke"
all_voting$name[all_voting$name == "Rod Benninson" &
									all_voting$division == "HUNTER"] <- "Rod Bennison"
all_voting$name[all_voting$name == "Garry Sloan" &
									all_voting$division == "CALARE"] <- "Garry Sloane"
all_voting$name[all_voting$name == "Kevin Glancey" &
									all_voting$division == "ROBERTSON"] <-  "Kevin Glancy"
all_voting$name[all_voting$name == "Adrian Deane" &
									all_voting$division == "FORDE"] <-  "Adrian Dean"
all_voting$name[all_voting$name == "Dikran Chabdijan" &
									all_voting$division == "BRUCE"] <-  "Dikran Chabdjian"
all_voting$name[all_voting$name == "Wolf Vogt" &
									all_voting$division == "LA TROBE"] <-  "Wolf Voigt"
all_voting$name[all_voting$name == "Robert Sims" &
									all_voting$division == "BOOTHBY"] <-  "Robert Simms"
all_voting$name[all_voting$name == "Graham Mathews" &
									all_voting$division == "BATMAN"] <-  "Graham Matthews"

all_voting <- all_voting %>%
	mutate(surname = word(name,-1),
				 surname = tolower(surname))

all_voting$surname[all_voting$name == "KEATING, Hon P" &
									 	all_voting$txt_file == "1987repsnsw"] <- "KEATINGHonP"
all_voting$surname[all_voting$name == "Paul KEATING" &
									 	all_voting$txt_file == "1987repsnsw"] <- "KEATINGHonP"
all_voting$surname[all_voting$name == "Keating, P" &
									 	all_voting$txt_file == "1987repsnsw"] <- "KeatingP_notPM"
all_voting$surname[all_voting$name == "Paul Keating" &
									 	all_voting$txt_file == "1987repsnsw"] <- "KeatingP_notPM"

all_voting$surname[all_voting$name == "Kate Butler" &
									 	all_voting$txt_file == "1996repsnsw"] <- "KateButler"
all_voting$surname[all_voting$name == "Butler, Kate" &
									 	all_voting$txt_file == "1996repsnsw"] <- "KateButler"
all_voting$surname[all_voting$name == "Kevin Butler" &
									 	all_voting$txt_file == "1996repsnsw"] <- "KevinButler"
all_voting$surname[all_voting$name == "Butler, Kevin" &
									 	all_voting$txt_file == "1996repsnsw"] <- "KevinButler"

all_voting$surname[all_voting$name == "Charles Abbott" &
									 	all_voting$txt_file == "1943repsnsw"] <- "CharlesAbbott"
all_voting$surname[all_voting$name == "Abbott, C" &
									 	all_voting$txt_file == "1943repsnsw"] <- "CharlesAbbott"
all_voting$surname[all_voting$name == "Abbott, J" &
									 	all_voting$txt_file == "1943repsnsw"] <- "JosephAbbott"
all_voting$surname[all_voting$name == "Joseph Abbott" &
									 	all_voting$txt_file == "1943repsnsw"] <- "JosephAbbott"
all_voting$surname[all_voting$name == "ABBOTT, J" &
									 	all_voting$txt_file == "1943repsnsw"] <- "JosephAbbott"

all_voting$surname[all_voting$name == "Barry O'Brien" &
									 	all_voting$txt_file == "1966repsvic"] <- "BarryOBrien"
all_voting$surname[all_voting$name == "O'Brien, B" &
									 	all_voting$txt_file == "1966repsvic"] <- "BarryOBrien"
all_voting$surname[all_voting$name == "John O'Brien" &
									 	all_voting$txt_file == "1966repsvic"] <- "JohnOBrien"
all_voting$surname[all_voting$name == "O'Brien, J" &
									 	all_voting$txt_file == "1966repsvic"] <- "JohnOBrien"

all_voting$surname[all_voting$name == "Brian Hughes" &
									 	all_voting$txt_file == "1993repsnsw"] <- "BrianHughes"
all_voting$surname[all_voting$name == "Hughes, R" &
									 	all_voting$txt_file == "1993repsnsw"] <- "BrianHughes"
all_voting$surname[all_voting$name == "David Hughes" &
									 	all_voting$txt_file == "1993repsnsw"] <- "DavidHughes"
all_voting$surname[all_voting$name == "Hughes, D" &
									 	all_voting$txt_file == "1993repsnsw"] <- "DavidHughes"

all_voting$surname[all_voting$name == "Ted Lindsay" &
									 	all_voting$txt_file == "1996repsqld"] <- "TedLindsay"
all_voting$surname[all_voting$name == "Lindsay, T" &
									 	all_voting$txt_file == "1996repsqld"] <- "TedLindsay"
all_voting$surname[all_voting$name == "Peter Lindsay" &
									 	all_voting$txt_file == "1996repsqld"] <- "PeterLindsay"
all_voting$surname[all_voting$name == "Lindsay, P" &
									 	all_voting$txt_file == "1996repsqld"] <- "PeterLindsay"
all_voting$surname[all_voting$name == "LINDSAY, P" &
									 	all_voting$txt_file == "1996repsqld"] <- "PeterLindsay"
all_voting$surname[all_voting$name == "Zillah Jackson" &
									 	all_voting$txt_file == "1996repsqld"] <- "ZillahJackson"
all_voting$surname[all_voting$name == "Jackson, Z" &
									 	all_voting$txt_file == "1996repsqld"] <- "ZillahJackson"

all_voting$surname[all_voting$name == "Terry Young" &
									 	all_voting$txt_file == "2019repsqld"] <- "TerryYoung"
all_voting$surname[all_voting$name == "T Young" &
									 	all_voting$txt_file == "2019repsqld"] <- "TerryYoung"
all_voting$surname[all_voting$name == "T YOUNG" &
									 	all_voting$txt_file == "2019repsqld"] <- "TerryYoung"

all_voting$surname[all_voting$name == "T Bell" &
									 	all_voting$txt_file == "2019repsqld"] <- "TraceyBell"
all_voting$surname[all_voting$name == "Tracey Bell" &
									 	all_voting$txt_file == "2019repsqld"] <- "TraceyBell"

all_voting$surname[all_voting$name == "Angie BELL" &
									 	all_voting$txt_file == "2019repsqld"] <- "AngieBell"
all_voting$surname[all_voting$name == "A BELL" &
									 	all_voting$txt_file == "2019repsqld"] <- "AngieBell"
...
all_voting$surname[all_voting$election_date == "1925-11-14" &
									 	all_voting$division == "RICHMOND" &
									 	all_voting$surname == "green" &
									 	all_voting$name == "Harry Green"] <- "HarryGreen"
all_voting$surname[all_voting$election_date == "1925-11-14" &
									 	all_voting$division == "RICHMOND" &
									 	all_voting$surname == "green" &
									 	all_voting$name == "H Green"] <- "HarryGreen"
all_voting$surname[all_voting$election_date == "1928-11-17" &
									 	all_voting$division == "MARIBYRNONG" &
									 	all_voting$surname == "fenton" &
									 	all_voting$name == "Arthur Fenton"] <- "ArthurFenton"
all_voting$surname[all_voting$election_date == "1929-10-12" &
									 	all_voting$division == "ECHUCA" &
									 	all_voting$surname == "hill" &
									 	all_voting$name == "Edward Hill"] <- "EdwardHill"
all_voting$surname[all_voting$election_date == "1931-12-19" &
									 	all_voting$division == "FREMANTLE" &
									 	all_voting$surname == "watson" &
									 	all_voting$name == "Keith Watson"] <- "KeithWatson"
all_voting$surname[all_voting$election_date == "1940-09-21" &
									 	all_voting$division == "EAST SYDNEY" &
									 	all_voting$surname == "gould" &
									 	all_voting$name == "Diana Gould"] <- "DianaGould"
all_voting$surname[all_voting$election_date == "1940-09-21" &
									 	all_voting$division == "EAST SYDNEY" &
									 	all_voting$surname == "gould" &
									 	all_voting$name == "D Gould"] <- "DianaGould"
all_voting$surname[all_voting$election_date == "1943-08-21" &
									 	all_voting$division == "LANG" &
									 	all_voting$surname == "mulcahy" &
									 	all_voting$name == "Daniel Mulcahy"] <- "DanielMulcahy"
all_voting$surname[all_voting$election_date == "1946-09-28" &
									 	all_voting$division == "BATMAN" &
									 	all_voting$surname == "brennan" &
									 	all_voting$name == "Allan Brennan"] <- "AllanBrennan"
all_voting$surname[all_voting$election_date == "1951-04-28" &
									 	all_voting$division == "COOK" &
									 	all_voting$surname == "hatfield" &
									 	all_voting$name == "Royston Hatfield"] <- "RoystonHatfield"
all_voting$surname[all_voting$election_date == "1954-05-29" &
									 	all_voting$division == "CUNNINGHAM" &
									 	all_voting$surname == "parkinson" &
									 	all_voting$name == "William Parkinson"] <- "WilliamParkinson"
all_voting$surname[all_voting$election_date == "1961-12-09" &
									 	all_voting$division == "KINGSFORD SMITH" &
									 	all_voting$surname == "cunningham" &
									 	all_voting$name == "John Cunningham"] <- "JohnCunningham"
all_voting$surname[all_voting$election_date == "1974-05-18" &
									 	all_voting$division == "SHORTLAND" &
									 	all_voting$surname == "morris" &
									 	all_voting$name == "David Morris"] <- "DavidMorris"
all_voting$surname[all_voting$election_date == "1975-12-13" &
									 	all_voting$division == "LANG" &
									 	all_voting$surname == "stewart" &
									 	all_voting$name == "John Stewart"] <- "JohnStewart"
all_voting$surname[all_voting$election_date == "1977-12-10" &
									 	all_voting$division == "HUGHES" &
									 	all_voting$surname == "johnson" &
									 	all_voting$name == "Kenneth Johnson"] <- "KennethJohnson"
all_voting$surname[all_voting$election_date == "1990-03-24" &
									 	all_voting$division == "BASS" &
									 	all_voting$surname == "smith" &
									 	all_voting$name == "Silvia Smith"] <- "SilviaSmith"
all_voting$surname[all_voting$election_date == "1990-03-24" &
									 	all_voting$division == "BASS" &
									 	all_voting$surname == "smith" &
									 	all_voting$name == "S Smith"] <- "SilviaSmith"
all_voting$surname[all_voting$election_date == "1993-03-13" &
									 	all_voting$division == "BASS" &
									 	all_voting$surname == "smith" &
									 	all_voting$name == "Silvia Smith"] <- "SilviaSmith"
all_voting$surname[all_voting$election_date == "1993-03-13" &
									 	...
									 	all_voting$name == "S Smith"] <- "SilviaSmith"
all_voting$surname[all_voting$election_date == "1993-03-13" &
									 	all_voting$division == "CANBERRA" &
									 	all_voting$surname == "kelly" &
									 	all_voting$name == "Sally Kelly"] <- "SallyKelly"
all_voting$surname[all_voting$election_date == "1993-03-13" &
									 	all_voting$division == "LOWE" &
									 	all_voting$surname == "woods" &
									 	all_voting$name == "Peter Woods"] <- "PeterWoods"
all_voting$surname[all_voting$election_date == "1993-03-13" &
									 	all_voting$division == "LOWE" &
									 	all_voting$surname == "woods" &
									 	all_voting$name == "P Woods"] <- "PeterWoods"
all_voting$surname[all_voting$election_date == "1996-03-02" &
									 	all_voting$division == "BASS" &
									 	all_voting$surname == "smith" &
									 	all_voting$name == "Silvia Smith"] <- "SilviaSmith"
all_voting$surname[all_voting$election_date == "1996-03-02" &
									 	...
									 	all_voting$surname == "smith" &
									 	all_voting$name == "S Smith"] <- "SilviaSmith"
all_voting$surname[all_voting$election_date == "1998-10-03" &
									 	all_voting$division == "BASS" &
									 	all_voting$surname == "smith" &
									 	all_voting$name == "Harvey Smith"] <- "HarveySmith"
all_voting$surname[all_voting$election_date == "1998-10-03" &
									 	all_voting$division == "BRADDON" &
									 	all_voting$surname == "thompson" &
									 	all_voting$name == "Gavin Thompson"] <- "GavinThompson"
all_voting$surname[all_voting$election_date == "1998-10-03" &
									 	all_voting$division == "BRADDON" &
									 	all_voting$surname == "thompson" &
									 	all_voting$name == "Thomson"] <- "GavinThompson"
all_voting$surname[all_voting$election_date == "1998-10-03" &
									 	all_voting$division == "FADDEN" &
									 	all_voting$surname == "smith" &
									 	all_voting$name == "Fay Smith"] <- "FaySmith"
all_voting$surname[all_voting$election_date == "1998-10-03" &
									 	all_voting$division == "FADDEN" &
									 	all_voting$surname == "smith" &
									 	all_voting$name == "F Smith"] <- "FaySmith"
all_voting$surname[all_voting$election_date == "1998-10-03" &
									 	all_voting$division == "HERBERT" &
									 	all_voting$surname == "lindsay" &
									 	all_voting$name == "Ted Lindsay"] <- "TedLindsay"
all_voting$surname[all_voting$election_date == "1998-10-03" &
									 	all_voting$division == "HERBERT" &
									 	all_voting$surname == "lindsay" &
									 	all_voting$name == "T Lindsay"] <- "TedLindsay"
all_voting$surname[all_voting$election_date == "1998-10-03" &
									 	all_voting$division == "HERBERT" &
									 	all_voting$surname == "smith" &
									 	all_voting$name == "Althea Smith"] <- "AltheaSmith"
all_voting$surname[all_voting$election_date == "1998-10-03" &
									 	all_voting$division == "HERBERT" &
									 	all_voting$surname == "smith" &
									 	all_voting$name == "A Smith"] <- "AltheaSmith"
all_voting$surname[all_voting$election_date == "1998-10-03" &
									 	all_voting$division == "MAYO" &
									 	all_voting$surname == "coombe" &
									 	all_voting$name == "Anthony Coombe"] <- "AnthonyCoombe"
all_voting$surname[all_voting$election_date == "2001-11-10" &
									 	all_voting$division == "BOWMAN" &
									 	all_voting$surname == "smith" &
									 	all_voting$name == "Fay Smith"] <- "FaySmith"
all_voting$surname[all_voting$election_date == "2001-11-10" &
									 	all_voting$division == "NEWCASTLE" &
									 	all_voting$surname == "williams" &
									 	all_voting$name == "Harry Williams"] <- "HarryWilliams"
all_voting$surname[all_voting$election_date == "2001-11-10" &
									 	all_voting$division == "NEWCASTLE" &
									 	all_voting$surname == "williams" &
									 	all_voting$name == "H Williams"] <- "HarryWilliams"
all_voting$surname[all_voting$election_date == "2004-10-09" &
									 	all_voting$division == "GWYDIR" &
									 	all_voting$surname == "anderson" &
									 	all_voting$name == "Michael Anderson"] <- "MichaelAnderson"
all_voting$surname[all_voting$election_date == "2004-10-09" &
									 	all_voting$division == "GWYDIR" &
									 	all_voting$surname == "anderson" &
									 	all_voting$name == "M Anderson"] <- "MichaelAnderson"
all_voting$surname[all_voting$election_date == "2004-10-09" &
									 	all_voting$division == "LOWE" &
									 	all_voting$surname == "murphy" &
									 	all_voting$name == "Shireen Murphy"] <- "ShireenMurphy"
all_voting$surname[all_voting$election_date == "2004-10-09" &
									 	all_voting$division == "LOWE" &
									 	all_voting$surname == "murphy" &
									 	all_voting$name == "S Murphy"] <- "ShireenMurphy"
all_voting$surname[all_voting$election_date == "2004-10-09" &
									 	all_voting$division == "PEARCE" &
									 	all_voting$surname == "mclean" &
									 	all_voting$name == "Donella Mclean"] <- "DonellaMclean"
all_voting$surname[all_voting$election_date == "2004-10-09" &
									 	all_voting$division == "PEARCE" &
									 	all_voting$surname == "mclean" &
									 	all_voting$name == "D Mclean"] <- "DonellaMclean"
all_voting$surname[all_voting$election_date == "2004-10-09" &
									 	all_voting$division == "WARRINGAH" &
									 	all_voting$surname == "kelly" &
									 	all_voting$name == "Edward Kelly"] <- "EdwardKelly"
all_voting$surname[all_voting$election_date == "2004-10-09" &
									 	all_voting$division == "WARRINGAH" &
									 	all_voting$surname == "kelly" &
									 	all_voting$name == "E Kelly"] <- "EdwardKelly"
all_voting$surname[all_voting$election_date == "2007-11-24" &
									 	all_voting$division == "BANKS" &
									 	all_voting$surname == "nguyen" &
									 	all_voting$name == "Don Nguyen"] <- "DonNguyen"
all_voting$surname[all_voting$election_date == "2013-09-07" &
									 	all_voting$division == "FLINDERS" &
									 	all_voting$surname == "clark" &
									 	all_voting$name == "David Clark"] <- "DavidClark"
all_voting$surname[all_voting$election_date == "2013-09-07" &
									 	all_voting$division == "FLINDERS" &
									 	all_voting$surname == "clark" &
									 	all_voting$name == "D Clark"] <- "DavidClark"
all_voting$surname[all_voting$election_date == "2013-09-07" &
									 	all_voting$division == "WAKEFIELD" &
									 	all_voting$surname == "musolino" &
									 	all_voting$name == "Tony Musolino"] <- "TonyMusolino"
all_voting$surname[all_voting$election_date == "2013-09-07" &
									 	all_voting$division == "WAKEFIELD" &
									 	all_voting$surname == "musolino" &
									 	all_voting$name == "?"] <- "TonyMusolino"
all_voting$surname[all_voting$election_date == "2015-09-19" &
									 	all_voting$division == "CANNING" &
									 	all_voting$surname == "smith" &
									 	all_voting$name == "Greg Smith"] <- "GregSmith"
all_voting$surname[all_voting$election_date == "2015-09-19" &
									 	all_voting$division == "CANNING" &
									 	all_voting$surname == "smith" &
									 	all_voting$name == "G Smith"] <- "GregSmith"
all_voting$surname[all_voting$election_date == "2016-07-02" &
									 	all_voting$division == "FORDE" &
									 	all_voting$surname == "spain" &
									 	all_voting$name == "Sally Spain"] <- "SallySpain"
all_voting$surname[all_voting$election_date == "2016-07-02" &
									 	all_voting$division == "WRIGHT" &
									 	all_voting$surname == "smith" &
									 	all_voting$name == "Rod Smith"] <- "RodSmith"
all_voting$surname[all_voting$election_date == "2016-07-02" &
									 	all_voting$division == "WRIGHT" &
									 	all_voting$surname == "smith" &
									 	all_voting$name == "R Smith"] <- "RodSmith"
...
all_voting_others_joined <- all_voting_others %>%
	left_join(all_voting_firsts,
						by = c("election_date", "division", "surname"))
...
all_voting <- all_voting %>%
	mutate(
		full_name_for_matching = str_to_lower(full_name),
...
all_voting <- all_voting %>%
	group_by(election_date, division) %>%
	mutate(
		winnerDummyOld = if_else(
			election_date < ymd("1918-12-14"),
			max(votes_count),
			as.integer(0)
		),
